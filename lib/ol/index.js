 ...
 * 获取几何对象的最大外接geohash
 * @param {Geometry} geom 
 * @param {Number} precision 最高精度
 * @returns {String} 返回一个编码 
 */
function getGeomMaxGeohash (geom, precision = 10) {
  let extent = geom.getExtent()
  if (extent) {
    let lb = ngeohash.encode(extent[1], extent[0], precision)
    let lt = ngeohash.encode(extent[3], extent[0], precision)
    let rb = ngeohash.encode(extent[1], extent[2], precision)
    let rt = ngeohash.encode(extent[3], extent[2], precision)
...
function testSingleLineIntersect (line1, line2) {
  ...
  for(let i = 0; i < line1Nodes.length - 1; i++) {
    ...
    for(let j = 0; j < line2Nodes.length - 1; j++) {
      ...
      if (relation.isIntersect(l1a, l1b, l2a, l2b)) {
        return true
      }
    }
  }
 ...
 * 超短线检查
 * @param {Array<Feature>} lineFeatures 
 * @param {Number} minLength 单位与源数据单位一致
 * @returns {Array<Feature> || null}
 */
function testShortLines (lineFeatures, minLength) {
  // todo 单位
  if (!minLength || minLength < 0) {
    minLength = 0.0001 // 单位°
  }
  if (lineFeatures && lineFeatures.length > 0) {
    let shortLines = lineFeatures.filter((line) => {
      let geom = line.getGeometry()
      if (geom instanceof LineString || geom instanceof MultiLineString) {
        return geom.getLength() < minLength
      }
      return false
    })
    return shortLines
 ...
 * @returns 返回到最近的线的距离小于tolerance且大于0的点
 */
function checkPointsNearLines (points, lines, tolerance = 0.0001) {
  let pointCodes = points.map((item) => {
    let coord = item.getGeometry().getCoordinates()
    let code = ngeohash.encode(coord[1], coord[0], 10)
    let turfPoint = new GeoJSON().writeFeatureObject(item)
    return {
      code: code,
      turfData: turfPoint
    }
  })
  let lineCodes = new Map()
  lines.forEach((item) => {
    let code = getGeomMaxGeohash(item.getGeometry())
    let turfLine = new GeoJSON().writeFeatureObject(item)
    if (lineCodes.get(code)) {
      let value = lineCodes.get(code)
      value.push(turfLine)
    } else {
      lineCodes.set(code, [turfLine])
    }
  })

  let result = []
  points.forEach((item, index) => {
    let code = pointCodes[index].code
    let pt = pointCodes[index].turfData
    let minDistance = tolerance
    for(let kv of lineCodes) {
      let key = kv[0]
      let lines = kv[1]
      let quit = false
      if (code.startsWith(key)) {
        for(let i = 0; i < lines.length;i++) {
          let line = lines[i]
          let distance = turf.pointToLineDistance(pt, line, {units: 'miles'}) // 注意： 距离单位m， 这里要求数据投影采用wgs84或者墨卡托投影
          if(distance === 0) {
            minDistance = 0
            quit = true
            break
    ...
    if (minDistance < tolerance && minDistance > 0) {
      result.push(item)
    }
  })
  return result
